			...
			<td>
				<select style="width:50px" id="ContentLayoutId" name="ContentLayoutId">
					<?php foreach ($contentLayouts as $contentLayout) {?>
						<option value="<?php echo $contentLayout['ContentLayout']['id']; ?>" <?php if (isset($_REQUEST["ContentLayoutId"])) { echo $contentLayout['ContentLayout']['id']==$_REQUEST["ContentLayoutId"]?"selected='selected'":""; } ?>><?php echo ($contentLayout['ContentLayout']['name']); ?></option>
					<?php } ?>
				</select>
			</td>
		</tr>
...
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('Content Page List', true), array('action' => 'index/'));?></li>
	</ul>
</div>
