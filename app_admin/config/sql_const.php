<?php
	...
	define('MEAL_TYPE_LIST_SQL', 'SELECT mtl.meal_type_id, mtl.name FROM language l JOIN meal_type_language mtl ON (l.iso_code = ? AND l.id = mtl.language_id AND isnull(l.deleted) AND isnull(mtl.deleted))');
	// 朝食タイプリスト取得用 第一引数 ISOコード
	define('BREAKFAST_TYPE_LIST_SQL', 'SELECT btl.breakfast_type_id, btl.name FROM language l JOIN breakfast_type_language btl ON (l.iso_code = ? AND l.id = btl.language_id AND isnull(l.deleted) AND isnull(btl.deleted))');
	...
	define('HOTEL_GRADE_LIST_SQL', 'SELECT hgl.hotel_grade_id, hgl.name FROM language l JOIN hotel_grade_language hgl ON (l.id = hgl.language_id AND l.deleted IS NULL AND hgl.deleted IS NULL AND l.iso_code = ?) ORDER BY hgl.name');
	// 部屋ベッドリスト取得用 第一引数 ISOコード
	define('ROOM_BED_LIST_SQL', 'SELECT rbl.room_bed_id, rbl.name FROM language l JOIN room_bed_language rbl ON (l.id = rbl.language_id AND l.deleted IS NULL AND rbl.deleted IS NULL AND l.iso_code = ?) ORDER BY rbl.name');
	// タバコリスト取得用 第一引数 ISOコード
	define('SMOKING_LIST_SQL', 'SELECT sl.smoking_id, sl.name FROM language l JOIN smoking_language sl ON (l.id = sl.language_id AND l.deleted IS NULL AND sl.deleted IS NULL AND l.iso_code = ?) ORDER BY sl.name');
